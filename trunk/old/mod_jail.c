...
static void jail_init(server_rec *s, pool *p)
{
    ...
    int mib[] = { CTL_KERN, KERN_SECURELVL };
	...
	if (chdir("/") == -1) {
	    ap_log_error(APLOG_MARK, APLOG_ERR, s, "mod_jail call chroot() failed.");
	}

	if (sysctl(mib, sizeof(mib)/sizeof(int), 0, 0, &cfg->jail_scrlevel, sizeof(cfg->jail_scrlevel)) == -1) {
...
static const char *set_jail_root(cmd_parms *cmd, void *__unused__, char *arg)
{
    ...
    if (!ap_is_directory(arg)) {
        return "jail_rootdir must exist and must be directory";
    }	
...
static const char *set_jail_addr(cmd_parms *cmd, void *__unused__, char *arg)
{
    ...
    if (inet_aton(arg, &in) == 0) {
	return "could not make sense of ip address";
    }
